  ...
  return (
    <div className="w-full">
      <GlassCard 
        ...
        className={`
          flex items-start justify-between p-3 transition-all group mb-2
          ${level === 0 ? 'bg-white/10' : 'bg-white/5 hover:bg-white/10'}
      ...
      >
        ...
        >
          ...
          <span className={`text-sm font-medium leading-snug ${!(hasSubsections || currentLevelInfo) ? 'ml-6' : ''} ${isSelected ? 'text-blue-400' : 'text-white'}`}>
            {item.title}
          </span>
        </div>
        ...
        <button
          ...
          className={`shrink-0 p-1.5 rounded-lg transition-colors ${isSelected ? 'text-blue-400' : 'text-white/40 hover:bg-white/20 group-hover:opacity-100 opacity-40'}`}
                ...
                <div className="mb-4 space-y-1">
                  ...
                  <ul className="list-disc list-inside space-y-1">
                    {currentLevelInfo.map((info, i) => (
                      <li key={i} className="text-xs text-white/80 leading-relaxed">{info}</li>
                    ))}
                  </ul>
                </div>
              )}
