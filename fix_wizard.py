...
# Let's find the start of the Content Area
start_marker = r'\{/\* Content Area \*/\}\s*<div className="flex-1 overflow-y-auto p-8 space-y-6 custom-scrollbar">'
end_marker = r'\{/\* Footer Navigation \*/\}'

match = re.search(start_marker + r'.*?' + end_marker, content, re.DOTALL)
if match:
    old_wizard = match.group(0)

    new_wizard = """{/* Content Area */}
